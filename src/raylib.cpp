...
void LoadGameFonts()
{
    ...
    if (g_titleFont.texture.id != 0)
        SetTextureFilter(g_titleFont.texture, TEXTURE_FILTER_BILINEAR);
    if (g_semiFont.texture.id != 0)
        SetTextureFilter(g_semiFont.texture, TEXTURE_FILTER_BILINEAR);
    if (g_regularFont.texture.id != 0)
        SetTextureFilter(g_regularFont.texture, TEXTURE_FILTER_BILINEAR);
...
{
    if (!actionBar.showSherlockAbilityInfo)
        return;
...
static void DrawLoadGameModal(GameManager &gm, bool &showLoadModal, Vector2 mousePos, float sw, float sh)
{
    float modalW = sw * 0.45f;
    float modalH = sh * 0.35f;
    float modalX = (sw - modalW) / 2.0f;
    float modalY = (sh - modalH) / 2.0f;
    Rectangle modalRect = {modalX, modalY, modalW, modalH};
    ...
    DrawRectangle(0, 0, (int)sw, (int)sh, Fade(BLACK, 0.75f));
    DrawRectangleRec(modalRect, GetColor(0x0B080CFF));
    DrawRectangleLinesEx(modalRect, 3, GetColor(0xE5C158FF));

    DrawText("SELECT SAVE SLOT", modalX + 30.0f, modalY + 20.0f, 20, GetColor(0xE5C158FF));

    auto slots = gm.getRecentSaveSlots();
    float btnY = modalY + 70.0f;

    for (const auto &slot : slots)
    {
        Rectangle slotBtn = {modalX + 30.0f, btnY, modalW - 60.0f, 45.0f};
        bool hover = CheckCollisionPointRec(mousePos, slotBtn);

        DrawRectangleRec(slotBtn, hover ? GetColor(0x28558FFF) : GetColor(0x151218FF));
        DrawRectangleLinesEx(slotBtn, 2, slot.exists ? GetColor(0xE5C158FF) : Fade(GetColor(0xE5C158FF), 0.3f));

        DrawText(slot.details.c_str(), slotBtn.x + 15.0f, slotBtn.y + 14.0f, 15, slot.exists ? WHITE : GRAY);

        if (slot.exists && hover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            if (gm.loadGame(slot.filename))
            {
                showLoadModal = false;
            }
        }
    ...
    Rectangle closeBtn = {modalX + (modalW - 100.0f) / 2.0f, modalY + modalH - 45.0f, 100.0f, 30.0f};
    bool hoverClose = CheckCollisionPointRec(mousePos, closeBtn);
    DrawRectangleRec(closeBtn, hoverClose ? GetColor(0x9E2230FF) : Fade(GetColor(0x9E2230FF), 0.5f));
    DrawRectangleLinesEx(closeBtn, 2, GetColor(0xE5C158FF));
    DrawText("CANCEL", closeBtn.x + 22.0f, closeBtn.y + 7.0f, 13, WHITE);
               ...
               character *sherlock, character *watson, int firstPlayer, hero *draculaHero, hero *sherlockHero)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (!showLoadModal && !gameOver && !actionBar.awaitingDefense && !mustDiscard)
        {
        ...
        if (!showLoadModal && !gameOver && !mustDiscard)
        {
        ...
        if (boardTex.id > 0) DrawTexturePro(boardTex, {0, 0, (float)boardTex.width, (float)boardTex.height}, mapDest, {0, 0}, 0.0f, WHITE);
        else DrawRectangleRec(mapDest, GetColor(0x0F0D12FF));

        ActionBar_DrawMapHighlights(actionBar, gm, board, mapDest, boardTex, actingChar, activeHero);ActionBar_DrawTargetingHighlights(actionBar, gm, board, mapDest, boardTex);
        ActionBar_DrawActorSelection(actionBar, gm, board, mapDest, boardTex);

        if (invTeam1) { for (int fogNode : invTeam1->getFogPositions()) DrawFogTokenOnNode(fogNode, board, mapDest, (float)boardTex.width, (float)boardTex.height, fogTex); }
        if (invTeam2) { for (int fogNode : invTeam2->getFogPositions()) DrawFogTokenOnNode(fogNode, board, mapDest, (float)boardTex.width, (float)boardTex.height, fogTex); }

        for (auto &mc : mapChars)
        {
            if (mc.c && mc.c->isalive()) DrawCharacterOnNode(mc.c, mc.tex, mc.color, mapDest.width, mc.sizeMul, (float)boardTex.width);
        }
        ...
        if (team1PortraitArt.id > 0) DrawTexturePro(team1PortraitArt, {0, 0, (float)team1PortraitArt.width, (float)team1PortraitArt.height}, dracRect, {0, 0}, 0.0f, WHITE);
        DrawRectangleLinesEx(dracRect, 3, GetColor(0x9E2230FF));

        if (dracula)
        {
            if (!dracula->isalive()) DrawText("[ DEAD ]", (int)dracRect.x + 30, (int)(dracRect.y + dracRect.height + 8), 10, RED);
            else DrawGothicHealthBar(dracRect.x, dracRect.y + dracRect.height + 8, dracRect.width, dracula->gethealth(), dracula->getMaxHp());
        }
        ...
        if (team2PortraitArt.id > 0) DrawTexturePro(team2PortraitArt, {0, 0, (float)team2PortraitArt.width, (float)team2PortraitArt.height}, sherRect, {0, 0}, 0.0f, WHITE);
        DrawRectangleLinesEx(sherRect, 3, GetColor(0x28558FFF));

        if (sherlock)
        {
            if (!sherlock->isalive()) DrawText("[ DEAD ]", (int)sherRect.x + 30, (int)(sherRect.y + sherRect.height + 8), 10, RED);
            else DrawGothicHealthBar(sherRect.x, sherRect.y + sherRect.height + 8, sherRect.width, sherlock->gethealth(), sherlock->getMaxHp());
        }
        ...
        if (showLoadModal)
        {
            DrawLoadGameModal(gm, showLoadModal, mousePos, sw, sh);
        }
    ...
    for (auto &tex : loadedCardTextures) UnloadTexture(tex);
    UnloadGameFonts();
    CloseWindow();
}