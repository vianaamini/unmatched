...
// --------------------------------------------------------------------------
// Special Ability button (bottom of each hero panel)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Gothic health bar with heart icon
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Fog token drawing (static placement, no animation)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Texture loader with fallback paths
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Load Game Modal (merged from first version)
// --------------------------------------------------------------------------
               ...
               character *sherlock, character *watson, int firstPlayer, hero *draculaHero, hero *sherlockHero)
{
    ...
    ActionBarState actionBar;
    if (draculaHero)
        actionBar.team1Label = HeroFactionLabel(draculaHero);
    if (sherlockHero)
        actionBar.team2Label = HeroFactionLabel(sherlockHero);
    ...
    if (activePlayerTurn == 1 && invTeam1)
    {
        invTeam1->onTurnStart();
        OpenVanishPromptIfNeeded(invTeam1);
    }
    if (activePlayerTurn == 2 && invTeam2)
    {
        invTeam2->onTurnStart();
        OpenVanishPromptIfNeeded(invTeam2);
    }
    ...
    while (!WindowShouldClose())
    {
        ...
        float gap = 12.0f;
        float totalBtnsWidth = 2 * btnWidth + gap;
        float startX = sw - totalBtnsWidth - 12.0f;
        Rectangle saveBtn = {startX, headerH * 0.5f - btnHeight / 2.0f, btnWidth, btnHeight};
        Rectangle loadBtn = {startX + btnWidth + gap, headerH * 0.5f - btnHeight / 2.0f, btnWidth, btnHeight};

        bool hoverSave = CheckCollisionPointRec(mousePos, saveBtn);
        bool hoverLoad = CheckCollisionPointRec(mousePos, loadBtn);

        // Save
        if (hoverSave && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !showLoadModal)
        {
        ...
        // Load
        if (hoverLoad && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !showLoadModal)
        {
            showLoadModal = true;
        }
        ...
        if (!showLoadModal && !gameOver && !actionBar.awaitingDefense && !mustDiscard)
        {
            ...
            if (CheckCollisionPointRec(mousePos, handBox) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                if (activePlayerTurn == 1)
                    showHandP1 = !showHandP1;
                else
                    showHandP2 = !showHandP2;
            }
        ...
        DrawRectangleRec(loadBtn, hoverLoad ? GetColor(0x2A2130FF) : GetColor(0x151218FF));
        DrawRectangleLinesEx(loadBtn, 2, GetColor(0x28558FFF));
        DrawTextCentered(GetSemiFont(), "LOAD", loadBtn.x + loadBtn.width / 2.0f, loadBtn.y + 8, 15, 0.6f, GetColor(0xE5C158FF));
        ...
        auto TurnLetter = [](hero *h) -> const char * {
            ...
            for (auto &ch : n)
                ch = std::tolower(ch);
            if (n.find("dracula") != std::string::npos)
                return "D";
            if (n.find("invisible") != std::string::npos)
                return "I";
            if (n.find("sherlock") != std::string::npos)
                return "S";
            return "?";
        };
        ...
        // ---------- Expanded hand modal ----------
        if (!actionBar.awaitingDefense && currentShowHand && activeHero && !showLoadModal)
        {
            auto hand = activeHero->gethand();
            std::string faction = HeroFactionLabel(activeHero);
