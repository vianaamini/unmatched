...
std::vector<std::string> Movement::getPossibleMovesWithBoost(
    character* character,
    int baseSteps,
    int boostValue,
    const std::vector<character*>& alliesList,
    ...
    int totalSteps = baseSteps + boostValue;
    return getPossibleMoves(character, totalSteps, alliesList, enemiesList);
}
...
void Movement::boost(character* character, const card* playedCard, ActionType currentAction) const {
