...
HeroSelectionResult HeroSelection::show(int firstPlayer)
{
    ...
    InitWindow(
        WIDTH,
        HEIGHT,
        "Unmatched - Hero Selection"
    );

    SetTargetFPS(60);

    Texture2D background =
        LoadTexture("assets/menu_bg.png");
    ...
    if (semiBoldFont.texture.id != 0)
        SetTextureFilter(
            semiBoldFont.texture,
            TEXTURE_FILTER_BILINEAR
        );
    ...
    {
        fighter.texture =
            LoadTexture(fighter.imagePath);

        if (fighter.texture.id != 0)
        {
            SetTextureFilter(
                fighter.texture,
                TEXTURE_FILTER_BILINEAR
    ...
    while (!WindowShouldClose())
    {
        ...
        )
        {
            std::string player1Hero =
                (firstPlayer == 1)
                    ? fighters[selectedFirst].name
                    : fighters[selectedSecond].name;
            ...
            for (auto& fighter : fighters)
                UnloadTexture(fighter.texture);

            UnloadFont(titleFont);
            UnloadFont(regularFont);
            UnloadFont(semiBoldFont);
        ...
        CenterText(
            regularFont,
            "SELECT ONE FIGHTER TO STEP INTO THE FOG",
            WIDTH / 2.0f,
            158,
            17,
            1.0f,
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (texture.id != 0)
            {
                ...
                {
                    imageArea.x +
                        (imageArea.width - imageWidth) / 2.0f,

                    imageArea.y +
                        (imageArea.height - imageHeight) / 2.0f,
    ...
    return {
        HeroSelectionResultType::Retreat,
        firstPlayer,
        "",
        ""
    };
