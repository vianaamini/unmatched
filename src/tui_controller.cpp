...
Element TuiController::createCardDisplay(const card &c, const string &type, const string &range, const string &effect, int index)
{
    ...
    return vbox({text("  [" + to_string(index) + "] " + c.get_name()) | bold | color(Color::Cyan),
...
Element TuiController::createMapDisplay()
{
    ...
    for (const auto &line : mapLines)
    {
        ...
        {
            if (dPos == "n1")
                displayLine.replace(pos, 2, "D");
            else if (sPos == "n1")
                displayLine.replace(pos, 2, "S");
            else if (wPos == "n1")
                displayLine.replace(pos, 2, "W");
        }
...
{
    return vbox({text(" GAME COMMANDS ") | bold | color(Color::Yellow) | center,
                 separator(),
                 text("move <location>"),
...
Element TuiController::createLocationInfo()
{
    ...
    return vbox({text(" LOCATION INFO ") | bold | color(Color::Cyan) | center,
                 separator(),
...
{
    return vbox({text(" LEGEND: ") | bold | color(Color::Yellow),
                 text("(D) Dracula"),
...
{
    return vbox({text(" ACTION MENU ") | bold | color(Color::Green) | center,
                 separator(),
                 hbox({
...
void TuiController::run()
{
    ...
    auto help_renderer = Renderer([&]
                                  { return vbox({text("  HOW TO PLAY & SYSTEM COMMANDS  ") | center | bold | color(Color::Cyan),
                                                 separator(),
                                                 ...
                                                 separator(),
                                                 vbox({text("• move <location>  - Relocate hero to a connected adjacent node (e.g., move n14)"),
                ...
                try {
                    ...
                    if (h && card_idx >= 1 && card_idx <= h->handsize()) {
                        ...
                        if (playedCard.gettype() == cardtype::attack && opponent) {
                            ...
                            if (actionSuccess) {
                                gamelogs.push_back(
                                    h->getname() + " attacked " + opponent->getname() +
                                    " with " + playedCard.get_name() + " for " +
                                    to_string(playedCard.getattack()) + " damage!"
                                );

                                if (!opponent->isalive()) {
                                    gamelogs.push_back(opponent->getname() +
                                        " has been defeated and removed from the game!");
