...
void LoadGameFonts() {
    ...
    g_titleFont   = LoadFontEx("assets/fonts/Cinzel-Black.ttf", 160, nullptr, 0);

    g_semiFont    = LoadFontEx("assets/fonts/Cinzel-SemiBold.ttf", 96, nullptr, 0);

    g_regularFont = LoadFontEx("assets/fonts/Cinzel-Regular.ttf", 88, nullptr, 0);
...
void DrawGothicHealthBar(float x, float y, float width, int currentHp, int maxHp) {
    ...
    DrawText(TextFormat("HP: %d/%d", currentHp, maxHp), (int)x, (int)y + 12, 13.5, GetColor(0xC2B6B9FF));
...
void DrawCharacterOnNode(character* c, Texture2D avatarTex, Color borderColor, float mapWidth, float sizeMultiplier, float refWidth) {
    ...
    if (avatarTex.id > 0) {
        ...
        DrawText(initial.c_str(), (int)renderX - 5, (int)renderY - 7, 18.9, WHITE);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (tex.id != 0) {
            ...
            if (nameSize.x > cardRect.width - 10.0f) {
                ...
                DrawTextEx(GetSemiFont(), cardName.c_str(), { cardRect.x + 5, cardRect.y + 12 }, 20.2, 0.4f, BLACK);

            } else {

                DrawTextEx(GetSemiFont(), cardName.c_str(), { cardRect.x + 5, cardRect.y + 12 }, 25.7, 0.5f, BLACK);
    ...
    for (int i = 0; i < cardCount; i++) {
        ...
        if (tex.id != 0) {
            ...
            if (nameSize.x > cardRect.width - 10.0f) {
                ...
                DrawTextEx(GetSemiFont(), cardName.c_str(), { cardRect.x + 5, cardRect.y + 12 }, 21.6, 0.4f, BLACK);

            } else {

                DrawTextEx(GetSemiFont(), cardName.c_str(), { cardRect.x + 5, cardRect.y + 12 }, 27, 0.5f, BLACK);
            ...
            DrawTextEx(GetRegularFont(), defLine.c_str(), { cardRect.x + 5, cardRect.y + cardRect.height - 26 }, 18.9, 0.3f, GetColor(0x3A1A1AFF));
...
Vector2 mousePos = GetMousePosition();
        ...
        if (dracula) {
            ...
            if (!dracula->isalive()) {
                ...
                DrawText("[ DEAD ]", (int)dracRect.x + (int)(dracRect.width / 2) - 30, (int)(dracRect.y + dracRect.height + 8), 13.5, RED);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (sisObjs[i]) {
                    ...
                    if (!sisObjs[i]->isalive()) {
                        ...
                        DrawText("[DEAD]", (int)sisBox.x + 2, (int)(sisBox.y + sisBox.height + 6), 10.8, RED);
        ...
        if (sherlock) {
            ...
            if (!sherlock->isalive()) {
                ...
                DrawText("[ DEAD ]", (int)sherRect.x + (int)(sherRect.width / 2) - 30, (int)(sherRect.y + sherRect.height + 8), 13.5, RED);
        ...
        if (watson) {
            ...
            if (!watson->isalive()) {
                ...
                DrawText("[DEAD]", (int)watsonRect.x + 2, (int)(watsonRect.y + watsonRect.height + 6), 10.8, RED);
        ...
        if (!actionBar.awaitingDefense && currentShowHand && activeHero) {
            ...
            for (int i = 0; i < cardCount; i++) {
                ...
                if (CheckCollisionPointRec(mousePos, cardRect) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                    ...
                    ActionBar_HandleCardClick(actionBar, gm, activeHero, hand[i], i, *showHandFlag, board, draculaHero);
                ...
                if (tex.id != 0) {
                    ...
                    DrawTextEx(GetRegularFont(), atkDefLine.c_str(), { cardRect.x + 5, cardRect.y + cardRect.height - 26 }, 18.9, 0.3f, GetColor(0x3A1A1AFF));
