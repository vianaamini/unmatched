...
void GameManager::addCharacter(character *c, int team) {
    ...
    if (d) d->setAllCharacters(&allCharacters);
    InvisibleMan *inv = dynamic_cast<InvisibleMan *>(c);
    if (inv) inv->setAllCharacters(&allCharacters);
...
bool GameManager::moveCharacter(character *c, const std::string &targetSpace, const card *boostCard) {
    ...
    if (InvisibleMan* invC = dynamic_cast<InvisibleMan*>(c)) {
        ...
        if (startOnFog && targetOnFog) reachable = true;
    }
    if (!reachable) {
        int moveSteps = c->getmovement();
        if (boostCard) moveSteps += boostCard->getboost();
        auto allies = getAllies(c);
        auto enemies = getEnemies(c);
...
bool GameManager::resurrectSister(const std::string &sisterName, int heroNode) {
    ...
    for (character *c : allCharacters) {
        if (c->getname() == sisterName) { sister = c; break; }
    }
    ...
    for (const auto &zone : zones) {
        ...
        for (const auto &space : spaces) {
            ...
            bool occupied = false;
            for (character *c : allCharacters) {
                if (c->isalive() && c->getx() == node) { occupied = true; break; }
            }
...
void GameManager::forceEndTurn() {
    turnManager.setActionsRemaining(0);
    turnManager.endTurn();
}
...
character *GameManager::findCharacterByName(const std::string &name) {
    ...
    for (auto c : allCharacters) {
        ...
        if ((lowerName == "inv" || lowerName == "invisible" || lowerName == "im") && cName.find("invisible") != std::string::npos) return c;
    }
    return nullptr;
}
...
bool GameManager::handleMove(const std::string &charIdentifier, const std::string &targetNodeStr) {
    ...
    if (charIdentifier.empty()) actor = getCurrentCharacter();
    else actor = findCharacterByName(charIdentifier);
...
bool GameManager::handleMove(character *actor, const std::string &targetNodeStr) {
    ...
    for (auto c : currentTeamChars) {
        if (c == actor) { belongsToCurrentTeam = true; break; }
    }
    if (!belongsToCurrentTeam) return false;
    if (getActionsRemaining() <= 0) return false;

    hero* activeHero = nullptr;
    for (auto c : currentTeamChars) {
        hero* h = dynamic_cast<hero*>(c);
        if (h && h->isalive()) { activeHero = h; break; }
    }
    if (activeHero) {
    ...
    if (!formattedNode.empty()) {
        if (formattedNode[0] == 'N' || formattedNode[0] == 'n') formattedNode[0] = 'n';
        else formattedNode = "n" + formattedNode;
    }
    ...
    if (turnHero) turnHero->useAction();
...
static std::vector<std::string> SplitFields(const std::string &line, char delim) {
    ...
    std::string field;
    for (char ch : line) {
        if (ch == delim) { fields.push_back(field); field.clear(); }
        else field += ch;
    }
...
bool GameManager::saveGame(const std::string &filename) const {
    ...
    for (auto c : team1) {
        if (dynamic_cast<dracula *>(c)) { team1Type = "DRACULA"; break; }
    }
    for (auto c : team2) {
        hero *h = dynamic_cast<hero *>(c);
        if (h && !dynamic_cast<InvisibleMan *>(c)) { team2Type = "SHERLOCK"; break; }
    }
    ...
    for (auto c : allCharacters) {
        if (!c) continue;
        outFile << c->getname() << "|" << c->gethealth() << "|"
                << c->getMaxHp() << "|" << c->getposition() << "|";
        InvisibleMan *inv = dynamic_cast<InvisibleMan *>(c);
        if (inv) {
            auto fog = inv->getFogPositions();
            outFile << "1|" << fog.size();
            for (int pos : fog) outFile << "," << pos;
            outFile << "|";
        } else {
            outFile << "0||";
        }
        ...
        if (h) {
            ...
            for (const auto &cd : hand) {
                ...
                for (auto &ch : effect) if (ch == '|' || ch == ';' || ch == '\n' || ch == '\r') ch = ' ';
                std::string cardName = cd.get_name();
                for (auto &ch : cardName) if (ch == '|' || ch == ';' || ch == '\n' || ch == '\r') ch = ' ';
                outFile << "|" << cardName << ";" << static_cast<int>(cd.gettype()) << ";"
                        << cd.getattack() << ";" << cd.getdefense() << ";" << cd.getboost() << ";"
                        << static_cast<int>(cd.getowner()) << ";" << effect;
            }
        } else {
            outFile << "0";
        }
...
bool GameManager::loadGame(const std::string &filename) {
    ...
    if (!std::getline(inFile, line)) return false;
    if (!std::getline(inFile, line)) return false;
    int turnNum = 0; try { turnNum = std::stoi(line); } catch (...) { inFile.close(); return false; }
    if (!std::getline(inFile, line)) return false;
    int teamNum = 0; try { teamNum = std::stoi(line); } catch (...) { inFile.close(); return false; }
    if (!std::getline(inFile, line)) return false;
    int phaseVal = 0; try { phaseVal = std::stoi(line); } catch (...) { inFile.close(); return false; }
    if (!std::getline(inFile, line)) return false;
    int actionsRem = 0; try { actionsRem = std::stoi(line); } catch (...) { inFile.close(); return false; }
    if (!std::getline(inFile, line)) return false;
    std::string currentCharName = line;
    if (!std::getline(inFile, line)) return false;
    int charCount = 0; try { charCount = std::stoi(line); } catch (...) { inFile.close(); return false; }
    ...
    for (int i = 0; i < charCount; ++i) {
        ...
        try { hp = std::stoi(fields[1]); maxHp = std::stoi(fields[2]); pos = std::stoi(fields[3]); isInv = std::stoi(fields[4]); } catch (...) { continue; }
        ...
        if (isInv) {
            ...
            if (inv && !fogField.empty()) {
                ...
                for (char ch : fogField) {
                    if (ch == ',') { if (!num.empty()) { fogVals.push_back(std::stoi(num)); num.clear(); } }
                    else num += ch;
                }
                if (!num.empty()) fogVals.push_back(std::stoi(num));
        ...
        if (h) {
            int handCount = 0; try { handCount = std::stoi(handField); } catch (...) { handCount = 0; }
            auto &hand = h->gethand();
            hand.clear();
            for (int k = 0; k < handCount && nextIdx < (int)fields.size(); ++k, ++nextIdx) {
                std::vector<std::string> cardParts = SplitFields(fields[nextIdx], ';');
                if (cardParts.size() < 7) continue;
                std::string cardName = cardParts[0];
                int atk = 0, def = 0, boost = 0, typeVal = 0, ownerVal = 0;
                try { typeVal = std::stoi(cardParts[1]); atk = std::stoi(cardParts[2]); def = std::stoi(cardParts[3]); boost = std::stoi(cardParts[4]); ownerVal = std::stoi(cardParts[5]); } catch (...) { continue; }
                std::string effect = cardParts[6];
                hand.push_back(card(cardName, static_cast<cardtype>(typeVal), atk, def, boost, static_cast<cardowner>(ownerVal), effect));
            }
            int deckCount = 0;
            if (nextIdx < (int)fields.size()) {
                try { deckCount = std::stoi(fields[nextIdx]); } catch (...) { deckCount = 0; }
                ++nextIdx;
            }
            ...
            for (int k = 0; k < deckCount && nextIdx < (int)fields.size(); ++k, ++nextIdx) {
                ...
                try { ctypeVal = std::stoi(cardParts[1]); catk = std::stoi(cardParts[2]); cdef = std::stoi(cardParts[3]); cboost = std::stoi(cardParts[4]); cownerVal = std::stoi(cardParts[5]); } catch (...) { continue; }
                std::string ceffect = cardParts[6];
                restoredDeck.push_back(card(cardName, static_cast<cardtype>(ctypeVal), catk, cdef, cboost, static_cast<cardowner>(cownerVal), ceffect));
            }
